      ...
      <vs-table ref="table" multiple v-model="selectedUser" pagination :max-items="itemsPerPage" search :data="products">
        ...
        <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
          ...
          <div class="flex flex-wrap-reverse items-center data-list-btn-container">
            ...
            <!-- TRASH -->
            <div class="btn-add-new p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-center text-lg font-medium text-base text-danger border border-solid border-danger" @click="trashDataEx">
          ...
          <template slot-scope="{data}">
            <tbody>
              <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                  ...
                  <vs-td>
                      <p class="de-email font-medium truncate">{{ tr.email }}<br>Biệt danh: {{ tr.nick_name }}</p>
                  </vs-td>

                <vs-td>
                  <p class="de-describe font-medium truncate" v-if="tr.from_e == 'paypal' && tr.to_e != 'paypal'">
                      Từ: <img width="20" :src="require('@/assets/images/sky/icon_bank/paypal-mini.png')">
                      {{  tr.from_e.toUpperCase() }}
                      <br>
                      Đến: <IconCrypto style="width: 20px" :coinname="getIconType(tr.to_e)" color="color" format="svg" />
                      {{  tr.to_e.toUpperCase() }}
                  </p>
                  <p class="de-describe font-medium truncate" v-else-if="tr.to_e == 'paypal' && tr.from_e != 'paypal'">
                      Từ: <IconCrypto style="width: 20px" :coinname="getIconType(tr.from_e)" color="color" format="svg" />
                      {{  tr.from_e.toUpperCase() }}
                      <br>
                      Đến: <img width="20" :src="require('@/assets/images/sky/icon_bank/paypal-mini.png')">
                      {{  tr.to_e.toUpperCase() }}
                  </p>
                ...
                <vs-td>
                  <p class="de-send">{{ getAmountDecimal(tr.from_e, tr.send) }}
                  </p>
                </vs-td>

                <vs-td>
                  <p class="de-receive" v-if="tr.status">{{ getAmountDecimal(tr.to_e, tr.receive) }}</p>
                ...
                <vs-td class="whitespace-no-wrap text-center">
                      <vx-tooltip v-if="tr.delete_status == 0" style="float: left" color="danger" text="Xóa">
                          <vs-button color="dark" type="line" icon-pack="feather" icon="icon-trash" @click.stop="deleteEx(tr.id, indextr, 1)"></vs-button>
                      </vx-tooltip>
                      <vx-tooltip v-else style="float: left" color="warning" text="Thu hồi">
                          <vs-button color="dark" type="line" icon-pack="feather" icon="icon-arrow-up-left" @click.stop="deleteEx(tr.id, indextr, 0)"></vs-button>
                      </vx-tooltip>
                </vs-td>
              </vs-tr>
            </tbody>
...
export default {
  ...
  methods: {
    ...
    trashDataEx(){
      ...
      AuthenticationService.getExListHisTrash()
      .then((resp) => {
        if(!resp.data.success){
            localStorage.removeItem('token');
            this.$router.push('/pages/login').catch(() => {})
    ...
    reloadList(){
        ...
        AuthenticationService.getExListHistory()
        .then((resp) => {
