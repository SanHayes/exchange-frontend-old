  ...
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    ...
    <VuePerfectScrollbar style="height: calc(var(--vh, 1vh) * 100 - 16px - 45px - 10px);" v-if="dataType != 'addMoney'" class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      ...
      <div class="p-6">
        ...
        <!-- NAME -->
        <vs-input label="Họ" v-model="dataFirstName" class="mt-5 w-full" name="item-first_name" />

        <vs-input label="Tên" v-model="dataLastName" class="mt-5 w-full" name="item-last_name" />
  ...
  computed: {
    ...
    isFormValid() {
        if(this.dataType != 'addMoney'){
            return !this.errors.any() && this.dataEmail && this.dataNick
        }else{
