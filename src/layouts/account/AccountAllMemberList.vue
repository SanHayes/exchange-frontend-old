        ...
        <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
          ...
          <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 mr-4 items-per-page-handler">
            <div class="p-4 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
              <span class="mr-2 text-black">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ products.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : products.length }} trong {{ queriedItems }}</span>
              <feather-icon icon="ChevronDownIcon" svgClasses="h-4 w-4" />
            </div>
            ...
            <tbody>
              <vs-tr :data="tr" :key="tr.id" v-for="(tr, indextr) in data">
                      ...
                      <p>
                          ...
                          *** Lời: <span class="italic font-bold" style="color: #00ff89">${{ formatPrice(tr.priceWin - tr.priceLose, 2) }}</span><br>
                          <vx-tooltip v-if="!tr.active" style="float: left"  text="Kích hoạt tài khoản">
                             <vs-button color="warning" class="px-3 py-2 text-sm" style="border-radius: 20px;" @click.stop.prevent="activeAccount(tr)"> Kích hoạt tài khoản</vs-button>
                          </vx-tooltip>
                ...
                <vs-td>
                  ...
                  <vs-button color="success" icon-pack="feather" icon="icon-zap" @click="showCapTren(tr.upline_id)">Xem cấp trên</vs-button>
                  <br>
