      ...
      <div class="p-6">
        ...
        <div class="vx-row">
          <form-wizard
            ...
            nextButtonText="Tiếp"
            :finishButtonText="`${!verify ? 'Xong' : 'Thay đổi'}`"
          ...
          >
            <tab-content class="mb-5">
              ...
              <div class="vx-row">
                ...
                <div class="vx-col w-full mt-5">
                  <div>Quốc gia</div>
                  <div class="select">
                    <select v-model="country" class="w-full">
                      <option
                        :key="index"
                        :value="item.value"
            ...
            <tab-content class="mb-5 upl">
              ...
              <small
                >1. Hãy đảm bảo rằng ảnh chụp toàn vẹn và rõ ràng, có định dạng
                JPG, PNG, JPEG.</small
              >
              <small
                >2. Hãy chụp ảnh Chứng minh thư/Hộ chiếu của bạn. Ảnh cần sáng
                và rõ ràng, không bị mất góc.</small
              >
              <div class="vx-row m-0 mb-3">
                ...
                <div class="w-1/2 mb-2">
                  <label
                    class="sample cursor-pointer"
                    :style="`background: url(${urlPassFront}) center center no-repeat;`"
                  >
                    <span class="iconUpload"></span>
                    <input
                      type="file"
                      ref="imgFront"
                      @change="successUploadFront($event.target.files)"
                      class="hidden"
              ...
              <div class="vx-row m-0 mb-3">
                ...
                <div class="w-1/2 mb-2">
                  <label
                    class="sample cursor-pointer"
                    :style="`background: url(${urlPassBack}) center center no-repeat;`"
                  >
                    <span class="iconUpload"></span>
                    <input
                      type="file"
                      ref="imgBack"
                      @change="successUploadBack($event.target.files)"
                      class="hidden"
                      accept="image/png, image/jpg, image/jpeg"
                    />
                  </label>
            ...
            <tab-content class="mb-5">
              <div class="vx-row">
                ...
                <div class="vx-col w-full">
                  <p class="text-center" v-if="!verify">
                    Nhấn Xong để hoàn thành. Chúng tôi sẽ xem xét thông tin của
                    bạn!
                  </p>
                  <p class="text-center" v-if="verify">
                    Tài khoản cả bạn đã được xác minh!
                  </p>
                </div>
              </div>
            </tab-content>
...
export default {
  ...
  methods: {
    ...
    formSubmitted() {
      ...
      AuthenticationService.updateXacMinhTK(obj).then((res) => {
        if (res.data.success) {
          this.forBack = null;
          this.forFront = null;
    ...
    submitDataUpdateMoney() {
      ...
      AuthenticationService.updatePriceMember(obj).then((resp) => {
          ...
          if (Object.entries(this.data).length == 0) {
            AuthenticationService.checkEmail(this.dataEmail).then((resp) => {
              if (resp.data.success) {
                ...
                AuthenticationService.createUser(obj).then((resp) => {
                  if (resp.data.success) {
                    return this.$vs.notify({
            ...
            // cập nhật tài khoản
            AuthenticationService.updateMember(obj).then((resp) => {
              if (resp.data.success) {
                return this.$vs.notify({
