              ...
              >
                <div class="flex items-center">
                  ...
                  <div class="mx-2">
                    ...
                    <div>
                      <small class="mt-1 whitespace-no-wrap time">{{
                        ntf.time
                      }}</small>
                    </div>
                  </div>
...
export default {
  ...
  methods: {
    ...
    readAllNoti() {
      this.countNews = 0;
      this.unreadNotifications = this.unreadNotifications.map((e) => ({
        ...e,
        views: 1,
      }));
...
<path id="Path_35319" data-name="Path 35319" d="M4.079,8.158A4.091,4.091,0,0,1,0,4.079a.482.482,0,0,1,.51-.51.482.482,0,0,1,.51.51A3.059,3.059,0,1,0,4.079,1.02a.482.482,0,0,1-.51-.51A.482.482,0,0,1,4.079,0a4.079,4.079,0,0,1,0,8.158Z" transform="translate(0 0)" fill="#fcfcfd"></path>
  ...
  mounted() {
    ...
    if (!connection) {
      connection = true;
      this.connection = new WebSocket(config.BASE_URL_SOCKET_NOTIFY);
...
.time {
  background: rgba(58, 63, 101, 0.4) !important;
  border-radius: 3px;
